...
export default function OperationLogList({ logs, onLogClick }: Props) {
  if (logs.length === 0) {
    return (
      <div className="text-center py-8 text-gray-400 text-sm">暂无操作记录</div>
    );
  }
      ...
      {logs.map((log) => {
        ...
        return (
          <button
            ...
            className="w-full flex items-center gap-3 bg-white rounded-xl px-4 py-3 shadow-sm active:bg-gray-50 transition-colors text-left"
          >
